...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Function make_clamp_noisy creates a clamp with a defined amplitude and noise standard deviation (noise_amp)}
...
\NormalTok{    noise0 }\OperatorTok{=}\NormalTok{ np.random.normal(}\FloatTok{0.}\NormalTok{, noise_amp, }\BuiltInTok{int}\NormalTok{( }\DecValTok{2000}\OperatorTok{*}\NormalTok{(delay }\OperatorTok{+}\NormalTok{ duration)}\OperatorTok{*}\FloatTok{1e-3}\NormalTok{))   }\CommentTok{# Noise bandwith is limited to 2000Hz}
\NormalTok{    noise }\OperatorTok{=}\NormalTok{ np.real(scfft.ifft(scfft.rfft(noise0), N))                  }\CommentTok{# Use fft and ifft to scale the number of points}
\NormalTok{    noise }\OperatorTok{=}\NormalTok{ (noise }\OperatorTok{-}\NormalTok{ np.mean(noise)) }\OperatorTok{*}\NormalTok{ noise_amp}\OperatorTok{/}\NormalTok{np.std(noise)                     }\CommentTok{# Rescale the noise}

    \CommentTok{# Create a clean clamp}
\NormalTok{    clean_step }\OperatorTok{=}\NormalTok{ np.array([}\DecValTok{0} \ControlFlowTok{if}\NormalTok{ t }\OperatorTok{<}\NormalTok{ delay }\ControlFlowTok{else}\NormalTok{ amp }\ControlFlowTok{for}\NormalTok{ t }\KeywordTok{in}\NormalTok{ np.linspace(}\DecValTok{0}\NormalTok{, delay }\OperatorTok{+}\NormalTok{ duration, N)])}
